...
def crawl_product_urls(
    ...
    *,
    ...
    if todo:
        ...
        try:
            if workers > 1:
                _crawl_pipelined(todo, results, fetcher, llm_provider, workers, checkpoint)
            else:
...
def _crawl_pipelined(
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        # Duyet future theo dung thu tu submit nen `results` duoc lap day tu
        # dau ve cuoi - checkpoint ghi ra luon la 1 tien do lien tuc, khong
        # phai tap hop lo cho rai rac.
